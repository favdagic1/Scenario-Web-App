...
async function loadScenario(scenarioId) {
    ...
    lines.forEach(line => {
        ...
        if (line.nextLineId !== null) {
            allNextIds.add(line.nextLineId);
        }
    });

    for (const line of lines) {
        if (!allNextIds.has(line.lineId)) {
            firstLineId = line.lineId;
            break;
        }
    }
    ...
    while (currentId !== null && lineMap[currentId] && !visited.has(currentId)) {
        visited.add(currentId);
        orderedContent.push(lineMap[currentId]);
        currentId = lineMap[currentId].nextLineId;
    }

    lines.forEach(line => {
        if (!visited.has(line.lineId)) {
            orderedContent.push(lineMap[line.lineId]);
        }
    });
...
async function addDelta(deltaData) {
    await Delta.create(deltaData);
}
...
app.get('/api/scenarios/:scenarioId/deltas', async (req, res) => {
    try {
        ...
        const formattedDeltas = filteredDeltas.map(delta => {
            const result = { type: delta.type, timestamp: delta.timestamp };

            if (delta.type === 'line_update') {
                result.lineId = delta.lineId;
                result.nextLineId = delta.nextLineId;
                result.content = delta.content;
            } else if (delta.type === 'char_rename') {
                result.oldName = delta.oldName;
                result.newName = delta.newName;
            }
...
app.get('/api/scenarios/:scenarioId/restore/:checkpointId', async (req, res) => {
    try {
        ...
        const relevantDeltas = deltas.filter(d => d.timestamp <= checkpoint.timestamp);

        for (let delta of relevantDeltas) {
            if (delta.type === 'line_update') {
                let lineIndex = state.content.findIndex(l => l.lineId === delta.lineId);
                ...
                if (lineIndex === -1) {
                    ...
                    state.content[lineIndex].text = delta.content || '';
                    state.content[lineIndex].nextLineId = delta.nextLineId;
                }
            } else if (delta.type === 'char_rename') {
                for (let line of state.content) {
                    if (line.text && line.text.includes(delta.oldName)) {
